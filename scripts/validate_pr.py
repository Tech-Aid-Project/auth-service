...
def validate_branch_and_pr_title(branch, pr_title):
    ...
    if not branch.startswith("TADocs-"):
        print(f"The branch '{branch}' does not start with the pattern 'TAAT-'.")
        return False
    # Verify if the branch contains with 'TADocs-'
    if "TAAT-" not in pr_title:
        print(f"The PR title '{pr_title}' does not contain the pattern 'TAAT-'.")
        return False
    
    print(f"Validation successful: The branch '{branch}' and the PR title '{pr_title}' follow the 'TAAT-' pattern.")
